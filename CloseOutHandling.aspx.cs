...
namespace CTMS
{
    public partial class CloseOutHandling : System.Web.UI.Page
    {
        ...
        private void GetStructure(GridView grd)
        {
            try
            {
                ...
                DataSet ds;
                ds = new DataSet();
                ds = dal.GetSetMaster(Action: "GET_DATA", MODULENAME: "5", SUBMODULENAME: "5.8");
                grd.DataSource = ds;
                grd.DataBind();
            }
            catch (Exception ex)
        ...
        protected void bntSaveComplete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Write("<script> alert('Record Updated successfully.');window.location='CloseOutHandling.aspx'; </script>");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
