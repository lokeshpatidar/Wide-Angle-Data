...
namespace CTMS
{
    public partial class CloseOutInspection : System.Web.UI.Page
    {
        ...
        private void GetStructure(GridView grd)
        {
            try
            {
                ...
                DataSet ds;
                ds = new DataSet();
                ds = dal.GetSetMaster(Action: "GET_DATA", MODULENAME: "5", SUBMODULENAME: "5.4");
                grd.DataSource = ds;
                grd.DataBind();
            }
            catch (Exception ex)
        ...
        protected void bntSaveComplete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Write("<script> alert('Record Updated successfully.');window.location='CloseOutInspection.aspx'; </script>");
            }
            catch (Exception ex)
            {
                lblErrorMsg.Text = ex.Message.ToString();
